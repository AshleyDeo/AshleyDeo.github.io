let currPattern; // tracks pattern
let scaleNum; //tracks scale
let spray; //pattern drawn constantly
let timeDragged;//count franes
let timeMoved;
...
function mouseMoved(){
    if (spray) {
        ...
        if (timeMoved > 5) { // every 5 frames draw
            push();
            translate(random(-200, 100), random(-200, 100)); // translate around pointer
            if (currPattern == 1) {
